...
pub(crate) fn step_param<const D: usize>(
    adam: &AdamScaled,
    lr: f64,
...
pub struct SplatTrainer {
    config: TrainConfig,
    /// Per-step multipliers of the exponential LR schedules:
    /// `lr(n) = lr_<param> * decay^(n-1)`.
    lr_mean_decay: f64,
    lr_opac_decay: f64,
    lr_scale_decay: f64,
...
impl SplatTrainer {
    ...
    pub async fn step_prepared(
        ...
        let sched_step = self.step_count as i32 - 1;
        let lr_mean =
            self.config.lr_mean * self.lr_mean_decay.powi(sched_step) * median_scale as f64;
        let lr_opac = self.config.lr_opac * self.lr_opac_decay.powi(sched_step);
        let lr_scale = self.config.lr_scale * self.lr_scale_decay.powi(sched_step);
